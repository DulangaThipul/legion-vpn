...
export default function DashboardTabs({ user: initialUser }: { user: any }) {
  ...
  let metaData = { alert: "", isPremium: false, payments: [] };
  ...
  // 🚀 Big Custom Popup State (Dismissible with OK button)
  const [showCenterAlert, setShowCenterAlert] = useState(Boolean(metaData.alert));
  ...
  // 🚀 Send Heartbeat Every 15 seconds so Admin knows Client is Online
  useEffect(() => {
    sendClientHeartbeat();
  ...
  useEffect(() => {
    // Load Payments from DB metadata
  ...
  // 🚀 PERSIST PAYMENT TO DATABASE SO ADMIN CAN SEE SLIP
  const handleConfirmOrder = async () => {
    ...
    try {
      ...
      // Call Server Action to save directly into Prisma DB
      ...
      if (res.success) {
        ...
        alert("Saved locally. Admin will review your payment.");
      }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "transparent", color: "#FFFFFF", paddingBottom: "100px", position: "relative" }}>
      ...
      {/* 🚀 SCREEN CENTER BIG CUSTOM POPUP MESSAGE MODAL */}
      {showCenterAlert && metaData.alert && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", backdropFilter: "blur(12px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 99999, padding: "1.5rem" }}>
          <div style={{ background: "#11111a", border: "1px solid rgba(99,102,241,0.5)", borderRadius: "20px", padding: "2.5rem 2rem", maxWidth: "500px", width: "100%", textAlign: "center", boxShadow: "0 20px 50px rgba(0,0,0,0.8)", animation: "fadeInUp 0.3s ease" }}>
            <div style={{ fontSize: "3.5rem", marginBottom: "1rem" }}>
              {metaData.alert.split(" ")[0] || "📢"}
            </div>
            <h2 style={{ color: "#FFF", fontSize: "1.5rem", marginBottom: "1rem" }}>Notice from Admin</h2>
            <p style={{ color: "#cbd5e1", fontSize: "1.1rem", lineHeight: 1.6, marginBottom: "2rem" }}>
              {metaData.alert.replace(/^[^\s]+/, "").trim()}
            </p>
            <button onClick={() => setShowCenterAlert(false)} style={{ background: "linear-gradient(90deg, #4f46e5, #7c3aed)", color: "#FFF", border: "none", padding: "0.9rem 3rem", borderRadius: "12px", fontSize: "1.1rem", fontWeight: "bold", cursor: "pointer", boxShadow: "0 10px 25px rgba(99,102,241,0.4)" }}>
              OK, I Understand
            </button>
          </div>
        </div>
          ...
          {activeTab === "configs" && (
            <div className="animate-fade-in flex flex-col gap-4">
              ...
              {user?.vpnStatus === "Suspended" && (
                <div style={{ background: "rgba(245,158,11,0.15)", border: "1px solid #f59e0b", padding: "1rem", borderRadius: "10px", color: "#f59e0b" }}>
                  ⚠️ Your account is currently in REVIEW. Configs will activate once verified by Admin.
                </div>
              )}
              {parsedConfigs.map((cfg, idx) => (
                <div key={idx} style={{ background: "#0c0c14", padding: "1.2rem", borderRadius: "12px", border: "1px solid rgba(255,255,255,0.08)" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.8rem" }}>
                    ...
                    <button onClick={() => { navigator.clipboard.writeText(cfg.code); alert("Copied!"); }} style={{ background: "#4f46e5", color: "#FFF", border: "none", padding: "0.4rem 0.8rem", borderRadius: "6px", cursor: "pointer" }}>Copy</button>
                  </div>
