...
// 🛡️ CRASH PREVENTION HELPER: Never throws Invalid Date Error
...
export default function AdminDashboardClient({ initialUsers }: { initialUsers: any[] }) {
  ...
  if (!mounted) {
    return (
      <div style={{ minHeight: "100vh", background: "#050505", display: "flex", alignItems: "center", justifyContent: "center", color: "#6366f1", fontSize: "1.2rem", fontWeight: "bold" }}>
        Loading Admin CRM...
      </div>
    );
  }
  ...
  const handleUpdateUser = async (userId: string, updates: any) => {
    try {
      ...
      if (response?.success) {
        ...
        // Fallback for local preview if server action is pending
        setUsers(prev => prev.map(u => (u.id === userId || u._id === userId) ? { ...u, ...updates } : u));
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#050505", color: "#FFF", paddingBottom: "50px", fontFamily: "system-ui, sans-serif" }}>
      ...
      {/* HEADER */}
      <header style={{ padding: "1.5rem 2rem", background: "rgba(15,15,24,0.9)", borderBottom: "1px solid rgba(255,255,255,0.05)", display: "flex", justifyContent: "space-between", alignItems: "center", position: "sticky", top: 0, zIndex: 50, flexWrap: "wrap", gap: "1rem" }}>
        ...
        <Link href="/dashboard" style={{ color: "#818cf8", textDecoration: "none", fontWeight: "bold", background: "rgba(99,102,241,0.15)", padding: "0.6rem 1.4rem", borderRadius: "8px" }}>
          ← Exit to Client Dashboard
        </Link>
      </header>

      <main style={{ display: "grid", gridTemplateColumns: "350px 1fr", gap: "2rem", padding: "2rem", maxWidth: "1600px", margin: "0 auto", height: "calc(100vh - 90px)" }}>
        ...
        <div style={{ background: "rgba(15,15,24,0.6)", borderRadius: "16px", border: "1px solid rgba(255,255,255,0.05)", display: "flex", flexDirection: "column", overflow: "hidden" }}>
          <div style={{ padding: "1.2rem", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
            <input
              ...
              style={{ width: "100%", padding: "0.8rem 1rem", borderRadius: "10px", background: "rgba(0,0,0,0.5)", border: "1px solid rgba(255,255,255,0.1)", color: "#FFF", outline: "none", boxSizing: "border-box" }}
            />
          </div>
              ...
              filteredUsers.map(u => {
                ...
                return (
                  <div 
                    ...
                    style={{ display: "flex", alignItems: "center", gap: "0.8rem", padding: "0.9rem", borderRadius: "12px", background: isSelected ? "rgba(99,102,241,0.2)" : "transparent", border: `1px solid ${isSelected ? "rgba(99,102,241,0.5)" : "transparent"}`, cursor: "pointer", transition: "0.2s", marginBottom: "0.4rem" }}
                  >
...
function UserDetailsPanel({ user, onUpdate }: { user: any, onUpdate: (id: string, updates: any) => void }) {
  ...
  useEffect(() => {
    // Generate online status safely on client mount
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
      ...
      <div style={{ background: "rgba(15,15,24,0.8)", padding: "1.8rem", borderRadius: "16px", border: "1px solid rgba(255,255,255,0.05)", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "1rem" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "1.2rem" }}>
          <img src={user.image || `https://ui-avatars.com/api/?name=${encodeURIComponent(user.name || 'User')}`} alt="Avatar" style={{ width: "75px", height: "75px", borderRadius: "50%", objectFit: "cover", border: "3px solid #6366f1" }} />
          <div>
            <h2 style={{ margin: "0 0 0.2rem 0", fontSize: "1.6rem", color: "#FFF", display: "flex", alignItems: "center", gap: "10px", flexWrap: "wrap" }}>
              {user.name || "Unknown User"} 
              {user.vpnStatus === "Banned" && <span style={{ background: "#ef4444", color: "#FFF", fontSize: "0.75rem", padding: "2px 8px", borderRadius: "8px" }}>BANNED</span>}
            </h2>
            <p style={{ margin: "0 0 0.5rem 0", color: "#9ca3af", fontSize: "0.95rem" }}>{user.email || "No email"}</p>
        ...
        {/* 2 & 6: Premium Toggle & Ban Button */}
        <div style={{ display: "flex", flexDirection: "column", gap: "0.8rem", alignItems: "flex-end" }}>
          <button onClick={togglePremium} style={{ background: user.isPremium ? "linear-gradient(90deg, #4f46e5, #7c3aed)" : "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.1)", color: "#FFF", padding: "0.6rem 1.2rem", borderRadius: "10px", cursor: "pointer", fontWeight: "bold", fontSize: "0.9rem" }}>
            {user.isPremium ? "⭐ Premium User (Active)" : "⚪ Set as Premium"}
          </button>
          
          {user.vpnStatus !== "Banned" ? (
            <button onClick={handleBanUser} style={{ background: "rgba(239,68,68,0.15)", color: "#ef4444", border: "1px solid rgba(239,68,68,0.3)", padding: "0.5rem 1.2rem", borderRadius: "8px", fontWeight: "bold", cursor: "pointer" }}>
              🚫 Ban Client
            </button>
          ) : (
            <button onClick={handleUnbanUser} style={{ background: "#22c55e", color: "#000", border: "none", padding: "0.5rem 1.2rem", borderRadius: "8px", fontWeight: "bold", cursor: "pointer" }}>
              ✅ Unban Client
            </button>
          )}
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: "1.5rem" }}>
        ...
        {/* ⏳ 4. DASHBOARD COUNTDOWN (ADD DAYS) */}
        <div style={{ background: "rgba(15,15,24,0.8)", padding: "1.5rem", borderRadius: "16px", border: "1px solid rgba(255,255,255,0.05)" }}>
          <h3 style={{ margin: "0 0 1rem 0", color: "#FFF", fontSize: "1.1rem" }}>⏳ Subscription Countdown</h3>
          
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1.2rem", background: "rgba(0,0,0,0.3)", padding: "0.9rem", borderRadius: "10px" }}>
            <div>
              <p style={{ margin: 0, color: "#9ca3af", fontSize: "0.8rem" }}>Remaining Days</p>
              <h2 style={{ margin: "0.2rem 0 0 0", color: daysLeft > 0 ? "#22c55e" : "#ef4444", fontSize: "1.8rem" }}>{daysLeft > 0 ? `${daysLeft} Days` : "Expired"}</h2>
            </div>
            {expiryDate && <p style={{ margin: 0, color: "#9ca3af", fontSize: "0.8rem" }}>Valid: {expiryDate.toLocaleDateString()}</p>}
          </div>
          ...
          <div style={{ display: "flex", gap: "8px" }}>
            <input 
              ...
              style={{ flex: 1, padding: "0.7rem", borderRadius: "8px", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", color: "#FFF", outline: "none" }}
            />
        ...
        {/* 💬 7. CUSTOM POPUP MESSAGE WITH EMOJI */}
        <div style={{ background: "rgba(15,15,24,0.8)", padding: "1.5rem", borderRadius: "16px", border: "1px solid rgba(255,255,255,0.05)" }}>
          <h3 style={{ margin: "0 0 1rem 0", color: "#FFF", fontSize: "1.1rem" }}>💬 Send Live Pop-up Message</h3>
          
          {user.alertMessage && (
            <div style={{ background: "rgba(245,158,11,0.1)", border: "1px dashed #f59e0b", padding: "0.8rem", borderRadius: "8px", marginBottom: "1rem" }}>
              <p style={{ margin: "0 0 0.3rem 0", fontSize: "0.75rem", color: "#f59e0b" }}>Active on screen:</p>
              <h4 style={{ margin: 0, color: "#FFF", fontSize: "0.95rem" }}>{user.alertMessage}</h4>
              <button onClick={handleClearMessage} style={{ marginTop: "6px", background: "none", border: "none", color: "#ef4444", fontSize: "0.75rem", cursor: "pointer", textDecoration: "underline", padding: 0 }}>Remove</button>
            </div>
          )}

          <div style={{ display: "flex", gap: "8px" }}>
            <select value={msgEmoji} onChange={(e) => setMsgEmoji(e.target.value)} style={{ padding: "0.7rem", borderRadius: "8px", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", color: "#FFF", outline: "none", cursor: "pointer" }}>
            ...
            <input 
              type="text" 
              placeholder="e.g. Payment issue, please retry..." 
              value={msgText} 
      ...
      {/* 📦 3. ASSIGN CONFIGS */}
      <div style={{ background: "rgba(15,15,24,0.8)", padding: "1.5rem", borderRadius: "16px", border: "1px solid rgba(255,255,255,0.05)" }}>
        <h3 style={{ margin: "0 0 1rem 0", color: "#FFF", fontSize: "1.1rem" }}>📦 Add New Config to the Client</h3>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr auto", gap: "10px", alignItems: "start" }}>
          ...
          />
          <button onClick={handleAddConfig} style={{ background: "#22c55e", color: "#000", border: "none", padding: "0.8rem 1.4rem", borderRadius: "8px", fontWeight: "bold", cursor: "pointer", height: "45px" }}>
            + Add Config
          </button>
        </div>
        ...
        <div style={{ marginTop: "1.2rem" }}>
          ...
          <textarea 
            ...
            style={{ width: "100%", padding: "0.9rem", borderRadius: "8px", background: "rgba(0,0,0,0.3)", border: "1px solid rgba(255,255,255,0.05)", color: "#818cf8", minHeight: "120px", fontFamily: "monospace", fontSize: "0.85rem", whiteSpace: "pre-wrap", outline: "none", boxSizing: "border-box" }} 
          />
        </div>
      </div>

      {/* 🧾 8. PAYMENT SLIP & HISTORY */}
      <div style={{ background: "rgba(15,15,24,0.8)", padding: "1.5rem", borderRadius: "16px", border: "1px solid rgba(255,255,255,0.05)" }}>
        <h3 style={{ margin: "0 0 1rem 0", color: "#FFF", fontSize: "1.1rem" }}>🧾 Payment Slips & History</h3>
        
        {userPayments.length === 0 ? (
          <div style={{ textAlign: "center", padding: "2rem", background: "rgba(0,0,0,0.2)", borderRadius: "10px" }}>
            <p style={{ color: "#9ca3af", margin: 0 }}>No payments or uploaded slips for this client yet.</p>
          </div>
        ) : (
